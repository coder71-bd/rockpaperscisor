...
function updateName(e) {
  if (e.target.value.length > 7) {
    alert("name can't be greater than 5 characters");
  }
  if (e.target.value.length <= 7) {
...
function frontToGame() {
  ...
  quotes.textContent = 'Click on any icons on the left to start';
}
...
function backText() {
  if (userScore >= 5) {
    if (playerName1.textContent.toLowerCase() === 'you') {
      winOrLoss.textContent = 'You won this round.🚀🚀🚀';
    } else {
      winOrLoss.textContent = `${playerName1.textContent}! You won this round.🚀🚀🚀`;
    }
  } else if (compScore >= 5) {
    winOrLoss.textContent = `Your enemy won this round. Try again.😢😢😢`;
  }
}
...
function removeAnimation() {
  ...
  if (computerRock.classList.contains('computer_rock_animation'))
    computerRock.classList.remove('computer_rock_animation');

  if (computerScissor.classList.contains('computer_scissor_animation'))
    computerScissor.classList.remove('computer_scissor_animation');

  if (quotes.classList.contains('quotes_animation'))
    quotes.classList.remove('quotes_animation');
}
